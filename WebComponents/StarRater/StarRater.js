class StarRater extends HTMLElement {
    ...
    createStars() {
        const createStar = (_, id) => {
            ...
            star.setAttribute('date-value', Number(id) + 1)
            star.innerHTML = '&#9733;'
    ...
    setRating(event) {
        ...
        event.currentTarget.getAttribute('date-value'))

    }

    ratingHover(event) {
        this.currentRatingValue = event.currentTarget.getAttribute('date-value')
        this.highlightRating()
    ...
    highlightRating() {
        this.stars.forEach(star => {
            star.style.color = 
            this.currentRatingValue >= star.getAttribute('date-value') 
            ? 'yellow' 
            : 'gray'
        })
    }
